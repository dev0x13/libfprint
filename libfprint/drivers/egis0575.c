...
is_last_pkt (FpDevice *dev)
{
  ...
  r = ((type == PKT_TYPE_INIT) && (num == (EGIS0575_INIT_TOTAL - 1)));
  r |= ((type == PKT_TYPE_REPEAT) && (num == (EGIS0575_REPEAT_TOTAL - 1)));
 ...
 */
static char
postprocess_frames (FpDeviceEgis0575 *self, guint8 * img)
{
  size_t mean[EGIS0575_IMGCOUNT] = {0, 0, 0, 0, 0};

  if (!self->background)
    {
      self->background = g_malloc (EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT);
      memset (self->background, 255, EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT);

      for (size_t k = 0; k < EGIS0575_IMGCOUNT; k += 1)
        {
          guint8 * frame = &img[(k * EGIS0575_IMGSIZE) + EGIS0575_RFMDIS * EGIS0575_IMGWIDTH];

          for (size_t i = 0; i < EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT; i += 1)
  ...
  for (size_t k = 0; k < EGIS0575_IMGCOUNT; k += 1)
    {
      ...
      for (size_t i = 0; i < EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT; i += 1)
        {
          if (frame[i] - EGIS0575_MARGIN  > self->background[i])
      ...
      mean[k] /= EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT;
    }
              ...
              if (where_finger_is & (1 << k))
                {
                  ...
                  memcpy (stripdata, (transfer->buffer) + (((k) * EGIS0575_IMGSIZE) + EGIS0575_IMGWIDTH * EGIS0575_RFMDIS), EGIS0575_IMGWIDTH * EGIS0575_RFMGHEIGHT);
...
recv_data_resp (FpiSsm *ssm, FpDevice *dev)
{
  ...
  fpi_usb_transfer_fill_bulk (transfer, EGIS0575_EPIN, EGIS0575_INPSIZE);

  transfer->ssm = ssm;
  transfer->short_is_error = TRUE;

  fpi_usb_transfer_submit (transfer, EGIS0575_TIMEOUT, NULL, data_resp_cb, NULL);
}
...
recv_cmd_resp (FpiSsm *ssm, FpDevice *dev)
{
  ...
  fpi_usb_transfer_fill_bulk (transfer, EGIS0575_EPIN, 512);

  transfer->ssm = ssm;
  //transfer->short_is_error = TRUE;

  fpi_usb_transfer_submit (transfer, EGIS0575_TIMEOUT, NULL, cmd_resp_cb, NULL);
}

static void
send_cmd_req (FpiSsm *ssm, FpDevice *dev, unsigned char *pkt, size_t pkt_size)
{
  FpiUsbTransfer *transfer = fpi_usb_transfer_new (dev);

  fpi_usb_transfer_fill_bulk_full (transfer, EGIS0575_EPOUT, pkt, pkt_size, NULL);
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case SM_REQ:
      if (self->pkt_type == PKT_TYPE_INIT)
        send_cmd_req (ssm, dev, init_pkts[self->pkt_num], EGIS0575_INIT_PKTSIZE);
      else
        send_cmd_req (ssm, dev, repeat_pkts[self->pkt_num], EGIS0575_REPEAT_PKTSIZE);
      break;
...
fpi_device_egis0575_class_init (FpDeviceEgis0575Class *klass)
{
  ...
  dev_class->full_name = "Egis Technology Inc. (aka. LighTuning) 0575";
  ...
  img_class->bz3_threshold = EGIS0575_BZ3_THRESHOLD; /* security issue */
}
